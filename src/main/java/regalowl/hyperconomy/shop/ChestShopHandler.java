	...
	public void handleHyperEvent(HyperEvent event) {
		if (event instanceof HBlockBreakEvent) {
			...
			try {
				...
				String pname = hevent.getHyperPlayer().getName();
				String line3 = "";
				String line4 = "";
				if (pname.length() > 12) {
			...
			try {
				HyperPlayer clicker = hevent.getClicker();
				ChestShop cs = hevent.getChestShop();
				...
				int slot = hevent.getClickedSlot();
				if (sign == null) return;
				...
				HItemStack clickedItem = hevent.getClickedItem();
				if (clickedItem == null) {
					event.cancel();
					return;
				}
