...
public class LegacyDatabaseUpdates {
	...
	public void applyLegacyUpdates(double version, SyncSQLWrite sw) {
		...
		if (version < 1.23) {
			hc.log().severe("-----------------------------------------------------");
			hc.log().severe("[HyperConomy]Shutting down.  This version of HyperConomy can't update from your previous version.  "
					+ "Please first update to an older build before using this one.  "
		...
		if (version < 1.31) {
			...
			removeFiles.add("items.yml");
			removeFiles.add("enchants.yml");
