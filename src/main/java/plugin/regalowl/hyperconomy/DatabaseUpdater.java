...
public class DatabaseUpdater {
	...
	public void createTables(SQLWrite sw, boolean copydatabase) {
		...
		if (!copydatabase) {
			...
			sw.convertExecuteSynchronously("INSERT INTO hyperconomy_settings (SETTING, VALUE, TIME) VALUES ('version', '"+hc.getEconomyManager().getDatabaseUpdater().getVersion()+"', NOW() )");
		}
	}
	...
	public void updateTables(QueryResult qr) {
		...
		if (qr.next()) {
			...
			if (version < 1.23) {
				...
				hc.getEconomyManager().getDatabaseUpdater().addUpdateAfterLoad(1.23);
			...
			if (version < 1.27) {
				...
				hc.getEconomyManager().getDatabaseUpdater().addUpdateAfterLoad(1.27);
		...
		hc.getSQLRead().syncRead(hc.getEconomyManager(), "load2", query, null);
	}
		...
		for (Double d:updateAfterLoad) {
			if (d.doubleValue() == 1.23) {
				...
				while (it.hasNext()) {
					...
					if (owner == null || owner == "") {
						owner = hc.getEconomyManager().getGlobalShopAccount().getName();
					}
					String type = "player";
					if (owner.equalsIgnoreCase(hc.getEconomyManager().getGlobalShopAccount().getName())) {
						type = "server";
					}
