...
public class Transaction {
			...
			if (amount > 0){
				...
				if (totalitems >= amount) {
					...
					if (!toomuch){
						...
						//Allows the sale to proceed if the number being sold doesn't exceed the maximum initial items that can be sold, the price is set to static, or if the initial period is set to false.					
						if (amount <= (maxi) || (items.getBoolean(name + ".price.static")) == true || (items.getBoolean(name + ".initiation.initiation") == false)) {
						...
						//Informs the player of the maximum number of items they can sell before reaching the hyperbolic pricing curve.
						} else {
							p.sendMessage(ChatColor.BLUE + "Currently, you can't sell more than " + (maxi) + " " + name + "!");
						}
	 ...
	 * 
	 */
	private double gettaxPaid(Double price) {
		try {
			FileConfiguration items = hc.getYaml().getItems();
	 ...
	 * 
	 */
	public void setAll(HyperConomy hyperc, int itemid, int itemdata, int itemamount, String itemname, Player player, Economy econ, Calculation cal, Enchant enc, Log lo, Account account) {
		hc = hyperc;
		id = itemid;
		data = itemdata;
		amount = itemamount;
